...
class DecodeUnitIo(implicit p: Parameters) extends CasinoBundle()(p)
{
...
class DecodeUnit(implicit p: Parameters) extends CasinoModule()(p)
{
...
class FetchSerializerResp(implicit p: Parameters) extends CasinoBundle()(p)
{
   val uops = Vec(DECODE_WIDTH, new MicroOp())
   val pred_resp = new BranchPredictionResp()
}
class FetchSerializerIO(implicit p: Parameters) extends CasinoBundle()(p)
{
...
class FetchSerializerNtoM(implicit p: Parameters) extends CasinoModule()(p)
{
...
class DebugBranchMaskGenerationLogicIO(implicit p: Parameters) extends CasinoBundle()(p)
{
   val branch_mask = UInt(width = MAX_BR_COUNT)
}

class BranchMaskGenerationLogic(val pl_width: Int)(implicit p: Parameters) extends CasinoModule()(p)
{
   val io = new Bundle
   {
