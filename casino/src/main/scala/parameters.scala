...
trait HasCasinoCoreParameters extends tile.HasCoreParameters
{
   ...
   val casinoParams: CasinoCoreParams = p(CasinoKey)
   require(xLen == 64)
   ...
   val ISSUE_WIDTH      = casinoParams.issueWidth
   ...
   val NUM_ROB_ENTRIES  = casinoParams.numRobEntries     // number of ROB entries (e.g., 32 entries for R10k)
   val NUM_LSU_ENTRIES  = casinoParams.numLsuEntries     // number of LD/ST entries
   val MAX_BR_COUNT     = casinoParams.maxBrCount        // number of branches we can speculate simultaneously
   val PHYS_REG_COUNT   = casinoParams.numPhysRegisters  // size of the unified, physical register file
   val FETCH_BUFFER_SZ  = casinoParams.fetchBufferSz     // number of instructions that stored between fetch&decode


   val enableFetchBufferFlowThrough = casinoParams.enableFetchBufferFlowThrough 
   ...
   val enableBrResolutionRegister = casinoParams.enableBrResolutionRegister
    
   //************************************
   // Issue Window
   
   val numIssueSlotEntries = casinoParams.numIssueSlotEntries
   val enableAgePriorityIssue = casinoParams.enableAgePriorityIssue 
   ...
   val ENABLE_BRANCH_PREDICTOR = casinoParams.enableBranchPredictor
   val ENABLE_BPD_UMODE_ONLY = casinoParams.enableBpdUModeOnly
   val ENABLE_BPD_USHISTORY = casinoParams.enableBpdUSModeHistory
   ...
   val tageParams = casinoParams.tage
   val gshareParams = casinoParams.gshare
   val gskewParams = casinoParams.gskew
   ...
   val ENABLE_COMMIT_MAP_TABLE = casinoParams.enableCommitMapTable
