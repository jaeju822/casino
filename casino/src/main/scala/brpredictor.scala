...
class BpdResp(implicit p: Parameters) extends CasinoBundle()(p)
{
...
class BpdUpdate(implicit p: Parameters) extends CasinoBundle()(p)
{
...
abstract class BrPredictor(fetch_width: Int, val history_length: Int)(implicit p: Parameters) extends CasinoModule()(p)
{
...
class BrobBackendIo(fetch_width: Int)(implicit p: Parameters) extends CasinoBundle()(p)
{
...
class BrobDeallocateIdx(implicit p: Parameters) extends CasinoBundle()(p)
{
   val brob_idx = UInt(width = BROB_ADDR_SZ)
}
...
class BrobEntryMetaData(fetch_width: Int)(implicit p: Parameters) extends CasinoBundle()(p)
{
...
class BrobEntry(fetch_width: Int)(implicit p: Parameters) extends CasinoBundle()(p)
{
...
class BranchReorderBuffer(fetch_width: Int, num_entries: Int)(implicit p: Parameters) extends CasinoModule()(p)
{
   val io = new CasinoBundle()(p)
   {
      // connection to CASINO's ROB/backend/etc.
