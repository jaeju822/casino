...
class LoadReqSlotIo(implicit p: Parameters) extends CasinoBundle()(p)
{
...
class LoadReqSlot(implicit p: Parameters) extends CasinoModule()(p)
{
...
class DCacheReq(implicit p: Parameters) extends CasinoBundle()(p)
{
...
class NackInfo(implicit p: Parameters) extends CasinoBundle()(p)
{
   val valid      = Bool()
...
class DCacheResp(implicit p: Parameters) extends CasinoBundle()(p)
{
   val data         = Bits(width = coreDataBits)
...
class DCMemPortIO(implicit p: Parameters) extends CasinoBundle()(p)
{
...
class DCacheShim(implicit p: Parameters) extends CasinoModule()(p)
{
...
//   val prefetch_req_val = prefetcher.io.cache.req.valid && Bool(ENABLE_PREFETCHING)
   ...
   assert (!(io.core.resp.valid && RegNext(io.dmem.xcpt.ma.ld) &&
      ...
      "Data cache returned an misaligned load exception, which CASINO handles elsewhere.")
   assert (!(io.core.resp.valid && RegNext(io.dmem.xcpt.ma.st) &&
      io.dmem.resp.bits.tag === RegNext(RegNext(io.dmem.req.bits.tag))),
      "Data cache returned an misaligned store exception, which CASINO handles elsewhere.")
   assert (!(io.core.resp.valid && RegNext(io.dmem.xcpt.pf.ld) &&
      io.dmem.resp.bits.tag === RegNext(RegNext(io.dmem.req.bits.tag))),
      "Data cache returned an faulting load exception, which CASINO handles elsewhere.")
   assert (!(io.core.resp.valid && RegNext(io.dmem.xcpt.pf.st) &&
      io.dmem.resp.bits.tag === RegNext(RegNext(io.dmem.req.bits.tag))),
      "Data cache returned an faulting store exception, which CASINO handles elsewhere.")
