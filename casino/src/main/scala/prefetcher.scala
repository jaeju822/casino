...
// they missed or not
class CoreRequest(implicit p: Parameters) extends CasinoBundle()(p)
{
...
class CacheReq(implicit p: Parameters) extends CasinoBundle()(p)
{
   val addr = UInt(width = coreMaxAddrBits)
}

class CacheIO(implicit p: Parameters) extends CasinoBundle()(p)
{
...
class Prefetcher(implicit p: Parameters) extends CasinoModule()(p)
{
   val io = new Bundle
   {
