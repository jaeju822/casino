...
class RobIo(machine_width: Int
            ...
            , num_fpu_ports: Int
            )(implicit p: Parameters)  extends CasinoBundle()(p)
{
   // Dispatch Stage
...
class CommitSignals(machine_width: Int)(implicit p: Parameters) extends CasinoBundle()(p)
{
...
class CommitExceptionSignals(machine_width: Int)(implicit p: Parameters) extends CasinoBundle()(p)
{
...
class FlushSignals(implicit p: Parameters) extends CasinoBundle()(p)
{
...
class Exception(implicit p: Parameters) extends CasinoBundle()(p)
{
   val uop = new MicroOp()
   val cause = Bits(width=log2Up(freechips.rocketchip.rocket.Causes.all.max))
...
class RobPCRequest(implicit p: Parameters) extends CasinoBundle()(p)
{
...
class DebugRobSignals(implicit p: Parameters) extends CasinoBundle()(p)
{
   val state = UInt()
         ...
         )(implicit p: Parameters) extends CasinoModule()(p)
{
   ...
   class DebugRobBundle extends CasinoBundle()(p) {
         val valid = Bool()
         val busy = Bool()
         val uop = new MicroOp()
         val exception = Bool()
...
//                                        Str(" "))))
         ...
         {
            println("  CASINO's Chisel printf does not support commit_width >= " + COMMIT_WIDTH)
         }
